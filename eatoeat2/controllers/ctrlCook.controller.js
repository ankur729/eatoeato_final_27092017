...
module.exports.add_cook_info = function (req, res, next) {
    ...
    db.cook_infos.save(
        ...
        , function (err, cook_details) {
            ...
            var to_no = parseInt(req.body.profile_detail.cook_contact);
            var message = "Dear, " + req.body.basic_detail.cook_name + " Welcome to eatoeato.com, please wait for the admin approval and to start adding food."


            db.sms_template_infos.find({ 'name': 'Signup Complete/ Cook' }, function (err, smstemplate_data) {


                smstemplate = smstemplate_data[0].body.replace("^^FIRST_NAME^^", req.body.basic_detail.cook_name);
                console.log(smstemplate);


                request("http://smsgate.idigitie.com/http-api.php?username=eatoeato&password=idid@1234&senderid=EATOET&route=1&number=" + parseInt(req.body.profile_detail.cook_contact) + "&message=" + smstemplate, function (error, response, body) {
                    if (!error && response.statusCode == 200) {
            ...
            var mailOptions = {
                ...
                to: req.body.profile_detail.cook_email, // list of receivers
                subject: 'Welcome To EatoEato ', // Subject line
                text: 'Please Activate Your EatoEato Account', // plain text body
                html: '<b>Your Account Has Been Created by, Please Click on Below Link to Verify your Account</b> <br> <a href="https://eatoeato.com:3000/#/verify-user-params/">' + randomstring.generate({ length: 100, charset: 'alphabetic' }) + '</a>' // html body
            };
    ...
    db.cook_infos.find(
        ...
        , function (err, cook) {
            ...
            if (err) {
                ...
                else if (cook[0].isDeactivate == "true") {
...
module.exports.cook_profile_update = function (req, res, next) {
    ...
    db.cook_infos.find({
        ...
        , function (err, data, lastErrorObject) {
            ...
            if (req.body.delivery_by == 'EatoEato') {
            ...
            else if (req.body.delivery_by == 'Self') {
...
module.exports.get_cook_details = function (req, res, next) {
    ...
    db.cook_infos.find({ _id: mongojs.ObjectId(req.body.cook_id) }, function (err, cook_details) {
        ...
        if (err) console.log(err);
            ...
            console.log('THIS IS COOK DETAILS');
            console.log(cook_details);

            if (cook_details.length > 0) {

                if (cook_details[0].hasOwnProperty('food_details')) {

                    if (cook_details[0].food_details.length > 0) {
            ...
            , function (err, data, lastErrorObject) {
                ...
                db.track_order_infos.update(
                    ...
                    , function (err, user, lastErrorObject) {
                        ...
                        db.order_infos.find({ 'order_id': order_id }, { user_id: 1, cook_id: 1 }, function (err, usdata) {

                            console.log('THIS IS USDATA');
                            console.log(usdata);

                            var usid = usdata[0].user_id;
                            var cookid = usdata[0].cook_id;
                            ...
                            db.notification_infos.save(
                                {
                                    ...
                                    message: usdata[0].username + " ,Your Order #" + order_id + " has been confirmed.",
                                    seenstatus: '0',
                                    ...
                                    db.user_infos.find({ _id: mongojs.ObjectId(usid.toString()) }, function (err, userdata) {

                                        db.cook_infos.find({ _id: mongojs.ObjectId(cookid.toString()) }, { cook_company_name: 1 }, function (err, cdata) {

                                            var cname = cdata[0].cook_company_name;
                                            var cook_contact = cdata[0].cook_contact;

                                            var us_name = userdata[0].username;
                                            db.sms_template_infos.find({ 'name': 'Order Confirmed/ User' }, function (err, smstemplate_data) {



                                                smstemplate = smstemplate_data[0].body.replace("^^FIRST_NAME^^", us_name);
                                                smstemplate = smstemplate.replace("^^RESTAURANT_NAME^^", cname);

                                                console.log('THIS FFFFFFFFFFFFFFFFF TEMPOLATE');
                                                console.log(smstemplate);
                                                sendsms(parseInt(userdata[0].phone), smstemplate);
                    ...
                    , function (err, user, lastErrorObject) {
                        ...
                        db.sms_template_infos.find({ 'name': 'Order Cancelled/ User' }, function (err, smstemplate_data) {
                            ...
                            db.order_infos.find({ 'order_id': order_id }, { user_id: 1, cook_id: 1 }, function (err, usdata) {
                                ...
                                console.log('THIS IS USDATA CANCELLED');
                                console.log(usdata);
                                ...
                                db.notification_infos.save(
                                    {
                                        ...
                                        notificationid: config.notificationObj.notification_id_user_order_cancelled,
                                        user_cook_id: usid,
                                        title: config.notificationTitleContent.notification_title_for_user_order_cancelled,
                                        message: usdata[0].username + " ,Your Order #" + order_id + " has been cancelled.",
                                        seenstatus: '0',
                                        date: moment(new Date()).format("DD/MM/YYYY"),
                                        datetime: Math.round(moment().toDate().getTime() / 1000),
                                        ...
                                        db.notification_infos.save(
                                            {
                                                ...
                                                console.log('SENDING NOTIFICATION TO ANDROID CANCELLED');
                                                //USER
                                                db.user_infos.find({ _id: mongojs.ObjectId(usid.toString()) }, function (err, userdata) {

                                                    console.log('THIS IS USER TOKEN CANCELLED');

                                                    //SENDING MOBILE SMS TO USER
                                                    db.sms_template_infos.find({ 'name': 'Order Cancelled/ User' }, function (err, smstemplate_data) {



                                                        smstemplate = smstemplate_data[0].body.replace("^^FIRST_NAME^^", userdata[0].username);
                                                    ...
                                                    sendMessageToUser(userdata[0].token, "Your Order #" + order_id + " has been cancelled.!", config.notificationTitleContent.notification_title_for_user_order_cancelled, config.notificationObj.notification_id_user_order_cancelled, 'data2');

                                                });
                                                //COOK
                                                db.cook_infos.find({ _id: mongojs.ObjectId(cookid.toString()) }, function (err, cookdata) {

                                                    console.log('THIS IS COOK TOKEN CANCELLED');

                                                    //SENDING MOBILE SMS TO COOK
                                                    ...
                                                    sendMessageToUser(cookdata[0].token, "Your Order #" + order_id + " has been cancelled.!", config.notificationTitleContent.notification_title_for_cook_order_cancelled, config.notificationObj.notification_id_user_order_cancelled, 'data2');
            ...
            , function (err, data, lastErrorObject) {
                ...
                db.track_order_infos.update(
                    ...
                    },
                    {
                        $push: {
                            'order_history': {
                                ...
                                "order_status": 'ready_for_del',
                                "order_comment": 'order ready to del',
                                "status_date": moment(new Date()).format("DD/MM/YYYY"),
                                "status_time": moment().toDate().getTime(),
                    ...
                    , function (err, user, lastErrorObject) {
                        ...
                        res.send({ 'status': 'ready_for_del' });

                        db.order_infos.find({ order_id: order_id }, { delivery_id: 1, user_id: 1, delivery_id: 1, cook_id: 1 }, function (err, dvcode) {

                            var del_code = dvcode[0].delivery_id;
                            var userid = dvcode[0].user_id;
                            //   var delcode = dvcode[0].delivery_id;

                            db.user_infos.find({ _id: mongojs.ObjectId(userid) }, function (err, userdata) {
                                ...
                                db.sms_template_infos.find({ 'name': 'Order Ready To Delivery/ User' }, function (err, smstemplate_data) {
                                    ...
                                    console.log('SMSSSSSSSSSSS TEMPLATE');
                                    console.log(smstemplate);
                                    request("http://smsgate.idigitie.com/http-api.php?username=eatoeato&password=idid@1234&senderid=EATOET&route=1&number=" + parseInt(userphone) + "&message=" + smstemplate, function (error, response, body) {
                                        ...
                                        console.log('SENDING NOTIFICATION TO ANDROID READY FOR DEL');

                                        db.notification_infos.save(
                                            {

                                                notificationid: config.notificationObj.notification_title_for_user_order_readyfordel,
                                                user_cook_id: userid,
                                                title: config.notificationTitleContent.notification_title_for_user_order_readyfordel,
                                                message: userdata[0].username + " ,Your Order #" + order_id + " is Ready For Delivery.!",
                                                seenstatus: '0',
                                                date: moment(new Date()).format("DD/MM/YYYY"),
                                                datetime: Math.round(moment().toDate().getTime() / 1000),
                                                ...
                                                sendMessageToUser(userdata[0].token, userdata[0].username + " ,Your Order #" + order_id + " is Ready For Delivery.!", config.notificationTitleContent.notification_title_for_user_order_readyfordel, config.notificationObj.notification_id_user_order_readyfordel, 'data2');
