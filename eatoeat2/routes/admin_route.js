...
function sendMessageToUserIOS(deviceId, message, title, notificationid, data2) {
    request({
        url: 'https://fcm.googleapis.com/fcm/send',
        method: 'POST',
        headers: {
...
router
    ...
    .post('/test-notify-ios', function (req, res, next) {
            ...
            body: JSON.stringify(
                {
                    ...
                    "to": 'd8ff79abbf6805a7aee79a61c4d9cdd5a65cce0266fad00c5f5a36b19266584e'
                }
            )
        }, function (error, response, body) {
            if (error) {
                ...
                console.log('Done IOS NOTIFICATION!');
                res.send('done IOS');
            }
        });
...
router
    ...
    .post('/del-boy-pickup-list', function (req, res, next) {
        ...
        console.log(req.body.delivery_boy_id);
        db.cook_infos.find({
...
router
    ...
    .post('/send-redirect-sms', function (req, res, next) {

        console.log(req.body);
            var to_no=parseInt(req.body.to_no);
            var msg="Your EatoEato OTP Verification Code is ";
            var num = Math.floor(Math.random() * 900000) + 100000;
            msg=msg+num;
            request("http://smsgate.idigitie.com/http-api.php?username=eatoeato&password=idid@1234&senderid=EATOET&route=1&number=" + to_no + "&message=" + msg, function (error, response, body) {
                if (!error && response.statusCode == 200) {

                    res.send({'status':'success','otpval':num});
                    console.log('SMS SEND') // Print the google web page.
                }
            });
    });
